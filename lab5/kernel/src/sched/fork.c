...
int copy_process(unsigned long flags, unsigned long fn, unsigned long arg,
    ...
    print_h((unsigned long)p);
    if (!p) return -1;

    struct pt_regs *childregs = task_pt_regs(p);
    memset((void *)childregs, 0, sizeof(struct pt_regs));
    memset((void *)&p->cpu_context, 0, sizeof(struct cpu_context));
    ...
    p->cpu_context.pc = (unsigned long)
        ret_from_fork;  // for the first time, pc is set to ret_from_fork
    p->cpu_context.sp = (unsigned long)childregs;

    task[pid] = p;
    p->pid = pid;
    print_string("\r\n[INFO] Process created with pid: ");
    print_d(pid);
finish:
    preempt_enable();
    return pid;
...
int move_to_user_mode(unsigned long pc) {
    ...
    unsigned long stack =
        (unsigned long)kmalloc(THREAD_SIZE);  // allocate new user stack
...
void kp_user_mode(
    unsigned long func) {  // Kernel process(func pass by argu) to user mode
    print_string("\r\nKernel process started. Switching to user mode...");
    int err = move_to_user_mode(func);
    if (err < 0) {
        print_string("Error while moving process to user mode\n\r");
    }
}
