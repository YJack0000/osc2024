...
struct file_operations tmpfs_file_operations = {tmpfs_write,   tmpfs_read,
                                                tmpfs_open,    tmpfs_close,
                                                tmpfs_lseek64, tmpfs_getsize};
struct vnode_operations tmpfs_vnode_operations = {tmpfs_lookup, tmpfs_create,
                                                  tmpfs_mkdir};
...
int tmpfs_create(struct vnode *dir_node, struct vnode **target,
    ...
    for (; child_idx <= MAX_DIR_ENTRY; child_idx++) {
        ...
        if (strcmp(child_inode->name, component_name) == 0) {
            puts("[ERROR] tmpfs create file exists\r\n");
            return -1;
        }
    }
